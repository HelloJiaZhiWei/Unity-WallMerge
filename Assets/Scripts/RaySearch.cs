...
public class RaySearch : MonoBehaviour
{
   ...
   // public List<Vector3> pathPoints = new List<Vector3>();
    ...
    private void Update() 
    {
        ...
        if(isCanMove && checkTime <= 0)
        {
            checkTime = 0.4f;
            currentPoint = transform.position;
            Vector3 offset = currentPoint - lastPoint;
            for(int i = 0;i<cornerPoints.Count;i++)
            {
                MeshPoint mp = new MeshPoint();
                mp.position = cornerPoints[i].position+offset;
                mp.normal = cornerPoints[i].normal;
                cornerPoints[i] = mp;
            }
